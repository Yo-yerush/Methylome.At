...
params:
  ...
  plot_gap_px: 14              # gap between plots in grids
  default_plot_cols: 1         # default columns for multi-plot sections
  pie_plot_cols: 3             # columns for Gain/Loss pies (CG/CHG/CHH)
...
emit_grid <- function(files, cols = params$default_plot_cols) {
...
emit_dir <- function(dir_path, recursive = FALSE, cols = params$default_plot_cols, pattern = NULL) {
  files <- list_media_files(dir_path, recursive = recursive, pattern = pattern)
  emit_grid(files, cols = cols)
}
...
</style>', as.integer(params$plot_max_width_px), as.integer(params$plot_gap_px)))
```

# Intro {.tabset}

**Comparison:** `r params$var2` vs `r params$var1`  
**Results folder:** `r exp_path`

## Key settings (quick)

- **CG / CHG / CHH:** cytosine methylation sequence contexts.
- **Bins (`binSize`):** genome is tiled into fixed windows to reduce per-site noise.
- **minCytosinesCount:** minimum informative cytosines per bin.
- **minReadsPerCytosine:** per-site coverage filter before summarizing bins.
- **minProportionDiff:** minimal ΔmC threshold (CG/CHG/CHH).
- **pValueThreshold:** statistical cutoff for significance.
...
if (isTRUE(params$run_PCA_plot)) {
  ...
  emit_dir(PCA_plots_path, recursive = FALSE, cols = 1)
}
...
if (isTRUE(params$run_total_meth_plot)) {
  ...
  emit_dir(meth_levels_path, recursive = FALSE, cols = params$default_plot_cols)
}

if (isTRUE(params$run_CX_Chrplot)) {
  cat("## Chromosome methylation profiles\n\n")
  cat("Methylation along chromosomes. Useful for detecting pericentromeric/heterochromatin-linked changes.\n\n")
  emit_dir(ChrPlot_CX_path, recursive = FALSE, cols = 1)
  if (dir.exists(ChrPlot_subCX_path)) {
    cat("#### subCX\n\n")
    emit_dir(ChrPlot_subCX_path, recursive = TRUE, cols = 1)
  }
}

if (isTRUE(params$run_TEs_distance_n_size)) {
  cat("## TE methylation vs TE size & centromere distance\n\n")
  emit_dir(TEs_distance_n_size_path, recursive = TRUE, cols = params$default_plot_cols)
}

if (isTRUE(params$total_meth_annotation)) {
  cat("## Total methylation across genome annotations\n\n")
  emit_dir(total_meth_annotation_path, recursive = TRUE, cols = params$default_plot_cols)
}

if (isTRUE(params$run_TF_motifs)) {
  cat("## TF motif analysis\n\n")
  emit_dir(TF_motifs_path, recursive = TRUE, cols = params$default_plot_cols)
}
```
...
if (isTRUE(params$analyze_DMRs)) {
  cat("## What is a DMR in this pipeline?\n\n")
  cat("DMRs are called on fixed genomic **bins** (`binSize`) to stabilize estimates across multiple cytosines.\n\n")
  cat("- binSize = ", params$binSize, " bp\n", sep="")
  cat("- minCytosinesCount = ", params$minCytosinesCount, "\n", sep="")
  cat("- minReadsPerCytosine = ", params$minReadsPerCytosine, "\n", sep="")
  cat("- minProportionDiff (CG,CHG,CHH) = ", paste(params$minProportionDiff, collapse=", "), "\n", sep="")
  cat("- pValueThreshold = ", params$pValueThreshold, "\n\n", sep="")

  # DMR density (if present)
  dens <- list.files(DMRs_analysis_path, pattern = "DMRs_Density.*\\.(png|svg|jpg|jpeg|gif)$", full.names = TRUE, ignore.case = TRUE)
  if (length(dens) > 0) {
    cat("## DMR density\n\n")
    emit_grid(dens, cols = 1)
  }

  # Gain/Loss pies side-by-side + other gain/loss plots
  cat("## Gain/Loss summaries\n\n")
  ...
  if (length(pie_files) > 0) emit_grid(pie_files, cols = params$pie_plot_cols)
  if (length(other_files) > 0) emit_grid(other_files, cols = params$default_plot_cols)

  # Chromosome distribution
  cat("## Chromosome distribution of DMRs\n\n")
  emit_dir(ChrPlots_DMRs_path, recursive = TRUE, cols = 1)

  # Genome annotation
  cat("## Genome annotation summaries\n\n")
  emit_dir(genome_ann_path, recursive = TRUE, cols = params$default_plot_cols)

  # Functional groups
  if (isTRUE(params$run_functional_groups)) {
    fg_dir <- file.path(genome_ann_path, "functional_groups")
    if (dir.exists(fg_dir)) {
      cat("## Functional groups overlap\n\n")
      emit_dir(fg_dir, recursive = TRUE, cols = params$default_plot_cols)
    }
  }

  # BigWig listing (no heavy loads)
  ...
  if (isTRUE(params$run_GO_analysis)) {
    cat("## GO analysis\n\n")
    emit_dir(file.path(exp_path, "GO_analysis"), recursive = TRUE, cols = params$default_plot_cols)
  }
  if (isTRUE(params$run_KEGG_pathways)) {
    cat("## KEGG pathways\n\n")
    emit_dir(file.path(exp_path, "KEGG_pathway"), recursive = TRUE, cols = params$default_plot_cols)
  }
}
```
