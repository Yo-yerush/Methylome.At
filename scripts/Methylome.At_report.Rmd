...
if (isTRUE(params$run_QC) && dir.exists(qc_dir_path)) {
  ...
  cat("#### Sample scatter plots\n\n")
  cat("Pairwise sample-vs-sample scatter (hexbin) plots on genomic tiles. Pearson r is shown per pair. Rows = contexts (CG / CHG / CHH), columns = sample pairs.\n\n")
  if (dir.exists(qc_scatter_path)) {
    scatter_files <- list_media_files(qc_scatter_path, pattern = "QC_scatter_")
    if (length(scatter_files) > 0) {
      emit_grid(scatter_files, cols = 1, max_height_px = params$plot_max_height_px * 1.5)
    } else {
      cat("_No scatter plots found._\n\n")
    }
  }

  cat("#### Sample correlation {.tabset}\n\n")
  ...
  if (dir.exists(qc_corr_path)) {
    ...
    if (length(corr_files) > 0) {
      for (cntx_c in c("CG", "CHG", "CHH")) {
        cntx_corr <- corr_files[grepl(paste0("QC_correlation_heatmap_", cntx_c, "_"), basename(corr_files))]
        if (length(cntx_corr) > 0) {
          cat(paste0("##### ", cntx_c, "\n\n"))
          emit_grid(cntx_corr, cols = 1, max_height_px = params$plot_max_height_px)
        }
      }
    } else {
