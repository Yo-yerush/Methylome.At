...
# [Methylome.At](https://github.com/Yo-yerush/Methylome.At) report: `r params$var2` *vs.* `r params$var1` {.tabset}
...
- [Methylome.At report: `r params$var2` *vs.* `r params$var1` {.tabset}](#methylomeat-report-r-paramsvar2-vs-r-paramsvar1-tabset)
    ...
    if (isTRUE(params$run_functional_groups)) {
        ...
        if (dir.exists(fg_dir)) {
            ...
            if (length(fg_files) == 0) {
                ...
                # Remove *_groups_volcano_suvh8_vs_wt pattern
                fg_files <- fg_files[!grepl("_groups_volcano_suvh8_vs_wt", basename(fg_files), ignore.case = TRUE)]
                # Move *_groups_barPlots_suvh8_vs_wt to front and rename to 'summary'
                bar_idx <- grepl("_groups_barPlots_suvh8_vs_wt", basename(fg_files), ignore.case = TRUE)
                bar_files <- fg_files[bar_idx]
                rest_files <- fg_files[!bar_idx]
                # For bar files, replace pattern with ' summary'
                bar_files_named <- setNames(
                    bar_files,
                    gsub("_groups_barPlots_suvh8_vs_wt", " summary", tools::file_path_sans_ext(basename(bar_files)), ignore.case = TRUE)
                )
                # Try to group by context keywords in filenames
