...
if (!is.na(log_path) && file.exists(log_path)) {
  ...
  colorize_log <- function(lines) {
    out <- vapply(lines, function(line) {
      ...
      line <- gsub(">", "&gt;", line, fixed = TRUE)
      # Color text inside []
      line <- gsub("\\[(.*?)\\]", "<span style='color:#007acc;font-weight:bold;'>[\\1]</span>", line)
      # Highlight "Error" / "Warning"
      line <- gsub("Error", "<span style='color:#cb0a0a; font-weight:bold;'>Error</span>", line, perl = TRUE)
      line <- gsub("Warning", "<span style='color:#e6a700; font-weight:bold;'>Warning</span>", line, perl = TRUE)
      # Color numbers (not inside tags)
      line <- gsub("(?<![&#;a-zA-Z])\\b([0-9]+\\.?[0-9]*)\\b", "<span style='color:#d32f2f;'>\\1</span>", line, perl = TRUE)
      # Color punctuation
      line <- gsub("([()'\"{}&lt;&gt;])", "<span style='color:#a259e6;'>\\1</span>", line, perl = TRUE)
      line
    }, character(1))
    paste(out, collapse = "\n")
  }
