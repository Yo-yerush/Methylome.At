...
try({
  Methylome.At_main(
    ...
    run_total_meth_plot = as.logical(configs[18]),
    run_CX_Chrplot = as.logical(configs[19]),
    run_TEs_distance_n_size = as.logical(configs[20]),
    total_meth_annotation = as.logical(configs[21]),
    run_TF_motifs = as.logical(configs[22]),
    run_functional_groups = as.logical(configs[23]),
    run_GO_analysis = as.logical(configs[24]),
    run_KEGG_pathways = as.logical(configs[25]),
    analyze_strand_asymmetry_DMRs = as.logical(configs[26]),
    analyze_DMVs = as.logical(configs[27]),
    analyze_dH = as.logical(configs[28]),
    run_TE_metaPlots = as.logical(configs[29]),
    run_GeneBody_metaPlots = as.logical(configs[30]),
    run_GeneFeatures_metaPlots = as.logical(configs[31]),
    gene_features_binSize = as.numeric(configs[32]),
    metaPlot.random.genes = as.numeric(configs[33])
    )
  # message("\n\nwarnings:\n", warnings())
})
