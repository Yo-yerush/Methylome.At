DMRs_ann_plots <- function(var1, var2, context) {
  ...
  for (ann.loop in plot_levels_loop) {
    DMRsReplicates_loop_path = paste0(context,"/",ann.loop,"_",context,"_genom_annotations.csv")
  ...
  #########################
  # plot
  svg(file = paste0(context,"_genom_annotations.svg"), width = 2.45, height = 2, family = "serif")
  ...
  svg(file = "legend_genom_annotations.svg", width = 1.34, height = 1.83, family = "serif")
  par(mar = c(0,0,2,0))
